...
sums i = sumsHelper 1 1
  where
    ...
    sumsHelper j currentSum
      | j > i     = []                
      | otherwise = currentSum : sumsHelper (j + 1) (currentSum + j + 1)
...
sorted :: [Int] -> Bool
sorted []       = True               -- Base case: an empty list is considered sorted
sorted [_]      = True               -- Base case: a singleton list is considered sorted
sorted (x:y:xs)
    | x <= y    = sorted (y:xs)     -- If x <= y, continue checking the rest of the list
    | otherwise = False              -- If x > y, the list is not sorted
...
sumsOf :: [Int] -> [Int]
sumsOf xs = sumsOfHelper 0 xs
  where
    sumsOfHelper :: Int -> [Int] -> [Int]
    sumsOfHelper _ []     = []            
    sumsOfHelper acc (y:ys) = acc' : sumsOfHelper acc' ys
      where
        acc' = acc + y
...
mymaximum bigger initial (x:xs) = if bigger x currentMax then x else currentMax
  where
