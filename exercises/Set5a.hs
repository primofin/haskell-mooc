...
data Person = PersonConstructor Int String
  deriving Show

-- fred is a person whose name is Fred and age is 90
fred :: Person
fred = PersonConstructor 90 "Fred"

-- getName returns the name of the person
getName :: Person -> String
getName (PersonConstructor age name) = name

-- getAge returns the age of the person
getAge :: Person -> Int
getAge (PersonConstructor age _) = age

-- setName takes a person and returns a new person with the name changed
setName :: String -> Person -> Person
setName name (PersonConstructor age oldName) =PersonConstructor age name

-- setAge does likewise for age
setAge :: Int -> Person -> Person
setAge age (PersonConstructor _age name) = PersonConstructor age name
...
get (Increase count) = count
get (Decrease count) = count

-- tick increases an increasing counter by one or decreases a
-- decreasing counter by one
tick :: UpDown -> UpDown
tick (Increase count) = Increase $ count + 1
tick (Decrease count) = Decrease $ count - 1

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (Increase count) = (Decrease count)
toggle (Decrease count) = (Increase count)
...
-- Helper function to calculate the average of two colors
average :: [Double] -> [Double] -> [Double]
average [] [] = []
average (x:xs) (y:ys) = ((x + y) / 2) : average xs ys

-- Helper function to subtract rgb values from 1
...
rgb Green = [0,1,0]
rgb Blue = [0,0,1]
rgb (Mix c1 c2) = average (rgb c1) (rgb c2)
rgb (Invert c) = invert (rgb c)
...
toNat z 
  | z < 0 = Nothing
  | otherwise = Just (toNatHelper z)
  where 
    toNatHelper 0 = Zero
    toNatHelper m = PlusOne (toNatHelper (m - 1))
...
prettyPrint End = ""
prettyPrint bin = reverse (prettyPrintHelper bin)

prettyPrintHelper :: Bin -> String
prettyPrintHelper End = ""
prettyPrintHelper (O b) = '0' : prettyPrintHelper b
prettyPrintHelper (I b) = '1' : prettyPrintHelper b
...
toBin n
  ...
  where
    toBinHelper 0 = End
    toBinHelper m
      | m `mod` 2 == 0 = O (toBinHelper (m `div` 2))
