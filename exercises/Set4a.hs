...
longest :: (Ord a, Foldable f) => f [a] -> [a]
longest = foldr1 cmp
  where
    cmp xs ys
...
freqs = foldr (\x freqMap -> Map.alter (Just . maybe 1 (+1)) x freqMap) Map.empty
------------------------------------------------------------------------------
